...
public interface UserRoleAssignmentRepository extends JpaRepository<UserRoleAssignment, Long> {
    ...
    @Query("SELECT ura FROM UserRoleAssignment ura " + "JOIN FETCH ura.role r "
    ...
    @Query("SELECT CASE WHEN COUNT(ura) > 0 THEN true ELSE false END FROM UserRoleAssignment ura " + "JOIN ura.role r "
